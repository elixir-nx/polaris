defmodule Optex.Optimizers do
  ...
  the update methods defined in `Optex.Updates`. Optex treats
  ...
  Custom optimizers are often created via the `Optex.Updates` API.
  ...
  defined elsewhere):
      ...
      defmodule Learning do
        ...
        defn update(params, optimizer_state, inputs, targets, update_fn) do
          ...
          {Optex.Updates.apply_updates(params, scaled_updates), new_optimizer_state, loss}
        end
      end

      {model_params, _key} = Nx.Random.uniform(key, shape: {784, 10})
      {init_fn, update_fn} = Optex.Optimizers.adam(0.005)
  ...
  For a simpler approach, you can also use optimizers with the training API:
        ...
        model
        |> Optex.Loop.trainer(:categorical_cross_entropy, Optex.Optimizers.adam(0.005))
        |> Optex.Loop.run(data, epochs: 10, compiler: EXLA)
