defmodule Polaris.UpdatesTest do
  use Polaris.Case
  doctest Polaris.Updates
  ...
  describe "scale_by_schedule" do
    test "constructs a stateful transformation" do
      params = %{a: Nx.tensor([1.0, 2.0, 3.0])}
      assert {init_fn, update_fn} = scale_by_schedule(Polaris.Schedules.polynomial_decay(1.0e-2))
      assert is_function(init_fn, 1)
      assert is_function(update_fn, 3)
    ...
    test "composes with itself" do
      ...
      assert {init_fn, update_fn} =
               scale_by_schedule(Polaris.Schedules.polynomial_decay(1.0e-2))
               |> scale_by_schedule(Polaris.Schedules.polynomial_decay(1.0e-2))
    ...
    test "composes with stateless transformation" do
      ...
      assert {init_fn, update_fn} =
               scale_by_schedule(Polaris.Schedules.polynomial_decay(1.0e-2)) |> scale(1.0e-2)
  ...
  describe "accumulate_gradients" do
    test "returns 0 updates, accumulated state, no updated parent state" do
      assert {init_fn, update_fn} = trace(nesterov: true) |> accumulate_gradients(2)
      params = %{a: Nx.tensor([1.0, 2.0])}
      ...
      assert_all_close(init_trace.a, new_trace.a)
